...
public struct Timer2: Timer8Bit {
    ...
    public static var :  {
        get {
            let mode = ( & 0b10000000) >> UInt8(7)
            return .init(rawValue: mode) ??
        }
        set {
             |= (newValue.rawValue & 0b00000001) << UInt8(7)
        }
    }
    /// TOIE2 –  
    @inlinable
    ...
    public static var overflowInterruptEnable: Bool {
        get {
            let flag = ( & 0b01000000) >> UInt8(6)
            return flag == 1
        }
        set {
             |= (newValue ? 1 : 0) & 0b00000001 << UInt8(6)
        }
    }
}