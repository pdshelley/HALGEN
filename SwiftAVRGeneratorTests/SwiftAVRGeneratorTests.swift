...
final class SwiftAVRGeneratorTests: XCTestCase {
    ...
    func testExportAllWritesCoreAVRPackageShape() throws {
        ...
        XCTAssertTrue(FileManager.default.fileExists(atPath: sourceDirectory.appendingPathComponent("module/Interrupts.swift").path))
        ...
        XCTAssertTrue(swift4p.contains("- Sources/CoreAVR/module/Interrupts.swift"))
        ...
        XCTAssertTrue(readme.contains("- Interrupts"))
    }

    func testInterruptsGeneratorGeneratesInterruptVectorEnum() throws {
        let device = try loadDevice(named: "ATmega328P")
        let loader = makeRepositoryDocsLoader()
        let generator = InterruptsGenerator()
        ...
        let file = try XCTUnwrap(files.first(where: { $0.fileName == "Interrupts.swift" }))
        let enumRange = try XCTUnwrap(file.content.range(of: "public enum InterruptVector: UInt8"))
        let structRange = try XCTUnwrap(file.content.range(of: "public struct Interrupts"))

        XCTAssertLessThan(enumRange.lowerBound, structRange.lowerBound)
        XCTAssertTrue(file.content.contains("/// RESET - External Pin, Power-on Reset, Brown-out Reset and Watchdog Reset"))
        XCTAssertTrue(file.content.contains("case reset = 0"))
        XCTAssertTrue(file.content.contains("/// INT0 - External Interrupt Request 0"))
        XCTAssertTrue(file.content.contains("case int0 = 1"))
        XCTAssertTrue(file.content.contains("/// TIMER2_COMPA - Timer/Counter2 Compare Match A"))
        XCTAssertTrue(file.content.contains("case timer2Compa = 7"))
        XCTAssertTrue(file.content.contains("/// SPM_Ready - Store Program Memory Read"))
        XCTAssertTrue(file.content.contains("case spmReady = 25"))
    }
